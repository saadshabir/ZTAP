...
PHASE5_ENVIRONMENT_MODE ?= release
PHASE5_EXPECTED_MIGRATION_RUN_ID ?=
PHASE5_EXPECTED_COMMIT ?=
PHASE5_EXPECTED_REF ?=
PHASE5_EXPECTED_WORKFLOW_RUN_ID ?=
PHASE5_EXPECTED_WORKFLOW_EVENT ?=
PHASE5_EXPECTED_WORKFLOW_PATH ?=
PHASE5_EXPECTED_MIGRATION_BRANCH ?=
...
PHASE5_VERIFY_FLAGS = $(if $(PHASE5_EXPECTED_RUN_ID),--run-id "$(PHASE5_EXPECTED_RUN_ID)") $(if $(PHASE5_ENVIRONMENT_FILE),--environment "$(PHASE5_ENVIRONMENT_FILE)" --environment-mode "$(PHASE5_ENVIRONMENT_MODE)") $(if $(PHASE5_EXPECTED_MIGRATION_RUN_ID),--migration-ci-run-id "$(PHASE5_EXPECTED_MIGRATION_RUN_ID)") $(if $(PHASE5_EXPECTED_COMMIT),--commit "$(PHASE5_EXPECTED_COMMIT)") $(if $(PHASE5_EXPECTED_REF),--ref "$(PHASE5_EXPECTED_REF)") $(if $(PHASE5_EXPECTED_WORKFLOW_RUN_ID),--workflow-run-id "$(PHASE5_EXPECTED_WORKFLOW_RUN_ID)") $(if $(PHASE5_EXPECTED_WORKFLOW_EVENT),--workflow-event "$(PHASE5_EXPECTED_WORKFLOW_EVENT)") $(if $(PHASE5_EXPECTED_WORKFLOW_PATH),--workflow-path "$(PHASE5_EXPECTED_WORKFLOW_PATH)") $(if $(PHASE5_EXPECTED_MIGRATION_BRANCH),--migration-ci-branch "$(PHASE5_EXPECTED_MIGRATION_BRANCH)") $(if $(PHASE5_EXPECTED_ENVIRONMENT_ARCH),--environment-arch "$(PHASE5_EXPECTED_ENVIRONMENT_ARCH)") $(if $(PHASE5_HOSTED_EBPF_DIR),--hosted-ebpf-dir "$(PHASE5_HOSTED_EBPF_DIR)") $(if $(PHASE5_HOSTED_CAPABILITY_DIR),--hosted-capability-dir "$(PHASE5_HOSTED_CAPABILITY_DIR)")
